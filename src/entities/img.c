...
/*   Updated: 2024/02/21 17:53:53 by astein           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_tile(t_cub *cub, int x, int y, t_img *src, t_img *dest,
		int pixel_width)
{
	int		i;
	int		j;
	...
	while (i < pixel_width)
	{
		...
		while (j < pixel_width)
		{
			...
			{
				dst_pxl = dest->addr + ((y + i) * dest->line_len + (x + j)
						* (dest->bpp / 8));
		...
		t_vector_dbl *vector_of_line, int color)
{
	...
	deltaX = vector_of_line->x / distance;
	deltaY = vector_of_line->y / distance;
	currentX = start_coordinates->x;
	currentY = start_coordinates->y;
	for (double i = 0; i <= distance; i++)
	{
		set_pixel_to_image(img, (int)floor(currentX), (int)floor(currentY),
				color);
		currentX += deltaX;
		currentY += deltaY;
	}
...
void	ini_img_2d(t_cub *cub, t_img *img)
{
	...
	img->height = cub->win.win_height;
	img->width = cub->win.win_width;
	img->mlx_img = mlx_new_image(cub->win.mlx, cub->win.win_width,
			cub->win.win_height);
	img->addr = mlx_get_data_addr(img->mlx_img, &(img->bpp), &(img->line_len),
			&(img->endian));
	img_size = img->height * img->width * sizeof(img->bpp);
